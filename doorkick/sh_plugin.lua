...
PLUGIN.author = "Thadah Denyse ported to Helix by FatherSquirrel"
PLUGIN.desc = "Allows Combine to kick doors."
			...
			if IsValid(ent) and ent:GetClass() == "prop_door_rotating" then
				...
				if dist > 60 and dist < 80 then
					...
					if blocked or table.HasValue(blocked, ent:EntIndex()) then
						...
						ix.util.Notify("You cannot kick this door!", client)
						
					end
					
				elseif dist < 60 then
				
					ix.util.Notify("You are too close to kick the door!", client)
				
				elseif dist > 80 then
					
					ix.util.Notify("You are too far to kick the door!", client)			
				end
			else
				ix.util.Notify("You are not looking to a valid door", client)
			end
		end
	end
})
